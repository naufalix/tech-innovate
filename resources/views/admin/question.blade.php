  ...
  <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
    ...
    <div>
      ...
      <div class="col-12 d-flex">
        ...
        <button class="ms-auto btn btn-primary d-none" data-bs-toggle="modal" data-bs-target="#tambah">Tambah</button>
      </div>
      <!--end::Heading-->
      <!--begin::Block-->
      <div class="my-5 table-responsive">
        <table id="myTable" class="table table-striped table-hover table-rounded border gs-7">
          ...
          <tbody>
            ...
            @php
              $date = date_create($q->created_at);
            @endphp
            <tr>
              <td>{{ $loop->iteration }}</td>
              <td><span class="badge badge-primary mb-2">{{ $q->code }}</span></td>
              <td>{{ $q->name }}</td>
              <td><span class="badge badge-success mb-2">{{ $q->y }}</span></td>
              <td><span class="badge badge-danger mb-2">{{ $q->n }}</span></td>
              <td>{{date_format($date,"M d, Y")}}</td>
              {{-- <td>
                <a href="#" class="btn btn-icon btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#edit" onclick="edit({{ $q->id }})"><i class="bi bi-pencil-fill"></i></a>
                <a href="#" class="btn btn-icon btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#hapus" onclick="hapus({{ $q->id }})"><i class="fa fa-times"></i></a>
              </td> --}}
            </tr>
            @endforeach
          </tbody>
